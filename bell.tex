...
?class (!meta:subTopClass){1,2} ?a1,?a2,?a3.
# Seien ?a1, ?a2, ?a3 Attribute des Typus Klasse, die keine Unterklassen von ?class sind

owl:Class ^a ?a1,?a2,?a3.FILTER(
?class!=?a1&&?class!=?a2&&?class!=?a3&&?a1<?a2&&?a2<?a3)
# Seien ?a1, ?a2, ?a3 ungleich zueinander und zu ?class
...
# Seien ?a1l, ?a2l, ?a3l die woertlichen Bezeichnungen von ?a1, ?a2, ?a3 in englischer Sprache
) GROUP BY ?class limit 1000
\end{lstlisting}
...
Der Variable ?class wird hier der Typus owl:class zugewiesen.
...
Hierbei ist ?class ist eine Kategorisierung, die eine Menge an Unterelementen verbindet.
...
Um nun die gewünschte Sprache auszuwählen, wird nun ein FILTER genutzt.
...
Dazu wird die Limitierung !subTopClass angewandt, die beschränkt, dass ?a1, ?a2 und ?a3 keine Unterklassen von ?class sein dürfen.
Außerdem wird über FILTER verhindert, dass sich die drei Distraktoren untereinander gleichen oder mit ?class übereinstimmen.
