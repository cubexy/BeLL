...
\todo{Mehr zu URIs und Classes mit Attributen erklären (Klasse A hat Attribut Definition und Label bspw.)}
...
Wie in Abschnitt 1.1.1 bereits beschrieben wurde, gibt es bei dem existierenden Prototypen 3 Typen von Strategien, die zur Generierung von Fragen genutzt werden:
\begin{itemize}
\item die DEFINITION-Query (A defines B),
\item die CONTAINS-Query (A contains B),
\item und die SUPPORTS-Query (A supports B).

Die drei vorhandenen Strategien generieren auf Basis dieser Queries Fragen.
\end{itemize}
...
Die Funktionsweise der Query ist hier relativ trivial: Als ersten Schritt wird der Variable ?class, also unserer Klasse, der Typus owl:class zugewiesen.
Dadurch werden alle Attribute der Ontologie, die das Attribut owl:class besitzen, als potentielle Werte für ?class angesehen.
Im nächsten Schritt wird einer zweiten Variable, ?cl, der Klarname der Klasse ?class in der gewünschten Sprache zugewiesen.

Das kann man sich wie folgt vorstellen: ?class ist eine Kategorisierung, die eine Menge an Unterelementen verbindet.
...
Um das Beispiel \enqoute{Skateboard} verständlich zu halten, habe ich mich hier anstelle der SNIK-Ontologie auf Wikidata bezogen.
Wikidata ist eine Ontologie, die einen Großteil der Informationen der Wissenssammlung Wikipedia zusammenfasst.
\enquote}Skateboard} ist hier als eigene Klasse mit dem Identifier \enquote{Q15783} gelistet. Sie besitzt unter anderem die Eigenschaft \enquote{definition}, die mehrsprachig vorliegt.
Das Unterelement besitzt also mehrere Attribute. 

Eine mögliche Fragestellung dieses Query-Typen könnte wie folgt aussehen:\\

\begin{tabular}[c]{|c|c|}
\hline
\multicolumn{2}{|c|}{Was wird hiervon definiert? \enquote{Ein X, gelegentlich verdeutscht auch Rollbrett genannt, ist ein Brett (Deck) mit zwei Achsen (Trucks) [...]}}\\ \hline
Boot & Lastkraftwagen\\\hline
\textcolor{green}{Skateboard} & Citybike\\\hline
\end{tabular}
\\\\\todo{Tabellen-Centering anpassen}

Neben der richtigen Antwort \enquote{Skateboard} werden hier noch die drei Distraktoren \enquote{Boot}, \enquote{Lastkraftwagen} und \enquote{Citybike} als Antwortmöglichkeit vorgegeben.
...
\end{figure}
\todo{Beschreiben des Ergebnisses der Query sowie ihrer Vorgehensweise.}
\chapter{Ergebnis}
\todo{
