...
\end{table}
Diese entstandenen Tripel werden letztendlich von RDF an SPARQL weitergereicht. 
\section{SPARQL}
\ac{SPARQL} ist ein recht neuer Standard für die Abfrage von in RDF spezialisierten Informationen sowie für die Darstellung der zugehörigen Resultate, siehe \cref{img:semanticwebstack1}. Es basiert auf einfachen RDF-Anfragen in Form von Graphmustern, also einer Menge von Tripeln, enthält jedoch auch erweiterte Funktionen für die Konstruktion komplexerer Anfragemuster, die Verwendung zusätzlich hinzufügbarer Filterbedingungen und für die Formatierung der schlussendlichen Ausgabe. Man könnte also SPARQL als Vokabular des semantischen Webs bezeichnen.
\begin{figure}
\centering
...
Das \ac{SNIK} ist ein vollendetes Projekt, welches Begriffe des Informationsmanagements sowie deren Beziehungen untereinander beschreibt. Dabei benutzt es eine abgeänderte Version des Semantic Web Stack (\cref{img:semanticwebstack2}). 
\begin{figure}
\centering
